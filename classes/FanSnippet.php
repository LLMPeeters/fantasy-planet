...
class FanSnippet extends FantasyBREAD {
    
    // Object properties
    public ?MongoDB\BSON\ObjectId $id;
    public ?string $name;
    public array|MongoDB\Model\BSONArray|null $tags;
    public ?string $text;
    ...
    public function updateMe( string $newName, array|MongoDB\Model\BSONArray $newTags, string $newText ) {
        
        // Has to use $set to update the given fields
        $updateFilter = array( '_id'=> $this->id );
        $updateDocument = array(
            '$set'=> array(
                'name'=> $newName, 'tags'=> $newTags, 'text'=> $newText
            ),
            '$push'=> array(
    ...
    public function __construct( string $name, array|MongoDB\Model\BSONArray $tags, string $text, ?MongoDB\BSON\ObjectId $id = null ) {
    ...
    static public function getExistingSnippets( array $filter = array(), array $options = array() ):array {
        ...
        foreach( $result as $snippet )
            $found[] = new FanSnippet( $snippet->name, $snippet->tags, $snippet->text, $snippet->_id );
